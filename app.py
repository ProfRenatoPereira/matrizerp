...
def imprimir_holerite(id):
    ...
    try:
        ...
        holerite = {"operador": func[1], "cargo": "Operador Industrial", "maquina": func[0], "bruto": salario_bruto, "inss": inss, "irrf": irrf, "vt": vt, "liquido": salario_bruto - inss - irrf - vt, "data": datetime.datetime.now().strftime("%d/%m/%Y"), "tipo": tipo.upper()}
    except Exception as e:
        holerite = None
    finally:
        cursor.close()
...
def almoxarifado():
    ...
    try:
        if request.method == 'POST':
            ...
            qtd = float(request.form.get('quantidade_compra', 0))
            if mat_sel in CATALOGO_MATERIAIS and qtd > 0:
                ...
                if existe:
                    cursor.execute("UPDATE materiais SET volume_disponivel = %s WHERE id = %s;", (float(existe[1]) + qtd, existe[0]))
                else:
    ...
    try:
        if request.method == 'POST':
            ...
            if cod and nome:
                ...
                conn.commit()
                flash('Produto de Engenharia Cadastrado!', 'success')
...
def faturar_pedido(id):
    ...
    try:
        ...
        conn.commit()
        flash('Receita operacional faturada com sucesso!', 'success')
    except Exception as e:
        conn.rollback()
    finally:
        cursor.close()
...
def pcp():
    ...
    try:
        if request.method == 'POST':
            ped_id = int(request.form.get('pedido_id'))
            maq_id = int(request.form.get('maquina_id'))
            tempo = int(request.form.get('tempo_minutos'))
    ...
    try:
        if request.method == 'POST':
            ...
            if acao == 'RESET_COMPLETO':
                cursor.execute("TRUNCATE TABLE usuarios, investimentos_imobiliarios, maquinas, materiais, produtos, estrutura_produto, formacao_precos, pedidos_vendas, ordens_processo, estoque_produtos RESTART IDENTITY CASCADE;")
                ...
                conn.commit()
                flash("Banco completamente resetado!", "danger")
                ...
                if u and s:
                    ...
                    conn.commit()
                    flash(f"Equipe '{u}' registrada!", "success")
