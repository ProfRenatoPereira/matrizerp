...
def inicializar_banco():
    ...
    try:
        ...
        # Tabelas Pedagógicas Estendidas
        ...
        if cursor.fetchone()[0] == 0:
            cursor.execute("INSERT INTO usuarios (usuario, senha) VALUES (%s, %s);", ("admin", generate_password_hash("admin123")))
        conn.commit()
    except Exception as e:
        conn.rollback()
        raise e
...
def calcular_caixa_disponivel(conn):
    ...
    try:
        ...
        cursor.close()
# ==============================================================================
# SISTEMA DE ROTAS FLASK - AUTENTICAÇÃO E SIMULAÇÃO
# ==============================================================================
...
# ==============================================================================
# OPERAÇÕES INTERNAS - RH, ATIVOS E INVENTÁRIO
# ==============================================================================
...
def imprimir_holerite(id):
    ...
    try:
        ...
        holerite = {"operador": func[1], "cargo": "Operador Industrial", "maquina": func[0], "bruto": round(salario_bruto, 2), "inss": round(inss, 2), "irrf": round(irrf, 2), "vt": round(vt, 2), "liquido": round(salario_bruto - inss - irrf - vt, 2), "data": datetime.datetime.now().strftime("%d/%m/%Y"), "tipo": tipo.upper()}
    except Exception as e:
        if conn:
            conn.rollback()
        holerite = None
...
def professor_painel_secreto():
    conn = conexao_pool.getconn()
    cursor = conn.cursor()
    url_reset = request.path == '/professor/resetar'
    try:
        acao = request.form.get('acao_docente') if request.method == 'POST' else None
        if acao == 'RESET_COMPLETO' or url_reset:
            cursor.execute("""
                TRUNCATE TABLE 
        ...
        if conn:
            conn.rollback()
        usuarios = []
        logger.error(f"Erro painel professor: {e}")
    finally:
        cursor.close()
        conexao_pool.putconn(conn)
